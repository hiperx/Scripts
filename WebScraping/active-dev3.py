...
with open("wynik-active-isa.txt", "w") as result_file:
    for hostname in hostnames:
        ...
        try:
            ...
            model_number_match = re.search(r"Hardware:\s+(\S+),", output, re.I)
            #model_number_match = re.search(r"Model\s+Number\s*:\s*(.+)", output, re.I)
            system_serial_numbers = re.findall(r"Serial\s+Number\s*:\s*(.+)", output, re.I)
